//gcc -m32 -w -g decode_sniff.c -o decode_sniff -lpcap
// if failing:
...
void decode_ip(const u_char *header_start) {
	...
	printf("\t((  Layer 3 ::: IP Header  ))\n");
        ...
        inet_ntop(AF_INET, &(ip_header->ip_src_addr), src_ip, INET_ADDRSTRLEN);

	//printf("\t( Source: %s\t", inet_ntoa(ip_header->ip_src_addr));
	//printf("Dest: %s )\n", inet_ntoa(ip_header->ip_dest_addr));
	char ip_str[INET_ADDRSTRLEN];
	inet_ntop(AF_INET, &(ip_header->ip_dest_addr), dest_ip, INET_ADDRSTRLEN);
